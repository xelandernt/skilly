...
export interface LauncherProcessLike extends PlatformProcessLike {
	...
	kill: (pid: number, signal: NodeJS.Signals) => boolean | undefined;
	pid: number;
	stderr?: {
...
export function resolveBinaryPath(
	options: Pick<LaunchOptions, "processLike" | "rootDir"> = {},
): string {
...
export function run(
	...
	child.once("error", (error: Error) => {
		writeError(
			processLike,
			`Failed to launch skilly from ${binaryPath}: ${error.message}`,
		);
		processLike.exit(1);
	});
